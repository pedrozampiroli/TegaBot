...
app.post("/receivecall", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
  ...
  let embed = new discord_js_1.default.MessageEmbed().setColor(prioridade.startsWith('BAIXA') ? '#33bd38' : prioridade.startsWith('MEDIA') ? '#e6cf00' : prioridade.startsWith('ALTA') ? '#6f03fc' : prioridade.startsWith('CR') ? '#cc0000' : '#33bd38').setTitle(`**Dados do chamado ${nroChamado}** \n**Cliente:** ${cliente} \n**Prioridade:** ${prioridade}`).setDescription(mensagem).setTimestamp();
  yield channel.send({
    embeds: [embed]
  });
  res.send("ok");
